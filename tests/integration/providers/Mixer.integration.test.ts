...
test("importOptions and exportOptions work as intended", () => {
  ...
  const mixerElement = mixer.getElement() as HTMLIFrameElement;
  expect(mixerElement).toBeInstanceOf(HTMLIFrameElement);
  ...
  expect(videoIFrameElement.outerHTML).toBe(mixerElement.outerHTML);
});
